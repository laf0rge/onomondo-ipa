...
static int dec_retr_notif_from_lst_res(struct ipa_es10b_retr_notif_from_lst_res *res, struct ipa_buf *es10b_res)
{
	...
	asn = ipa_es10b_res_dec(&asn_DEF_RetrieveNotificationsListResponse, es10b_res, "RetrieveNotificationsList");
	if (!asn)
		return -EINVAL;
	...
	case RetrieveNotificationsListResponse_PR_notificationsListResultError:
		...
		IPA_LOGP_ES10B("RetrieveNotificationsList", LERROR, "function failed with error code %ld=%s!\n",
			       res->notif_lst_result_err, ipa_str_from_num(error_code_strings,
									   res->notif_lst_result_err, "(unknown)"));
		break;
	default:
		IPA_LOGP_ES10B("RetrieveNotificationsList", LERROR, "unexpected response content!\n");
									...
									*req)
{
	...
	es10b_req =
	    ipa_es10b_req_enc(&asn_DEF_RetrieveNotificationsListRequest, &req->req, "RetrieveNotificationsList");
	if (!es10b_req) {
		IPA_LOGP_ES10B("RetrieveNotificationsList", LERROR, "unable to encode ES10b request\n");
		goto error;
	}

	es10b_res = ipa_euicc_transceive_es10x(ctx, es10b_req);
	if (!es10b_res) {
		IPA_LOGP_ES10B("RetrieveNotificationsList", LERROR, "no ES10b response\n");
		goto error;
	}
...
{
	IPA_ES10B_RES_FREE(asn_DEF_RetrieveNotificationsListResponse, res);
}
